import { createFileRoute } from '@tanstack/react-router';
import classnames from 'classnames/bind';
import { useEffect, useState } from 'react';

import Storage from '@/assets/image/cropStorage.png';
import MyCrops from '@/components/Crop/MyCrops';

import styles from './index.module.scss';

const cx = classnames.bind(styles);

export const Route = createFileRoute('/_layout/cropStorage/')({
  component: CropStoragePage,
});

function CropStoragePage() {
  const [nickName, setNickName] = useState(''); // 사용자 닉네임
  const [myCrops, setMyCrops] = useState([]); // 나의 농작물 데이터
  const [receivedCrops, setReceivedCrops] = useState([]); // 받은 농작물 데이터
  const [isMyCrops, setIsMyCrops] = useState(true); // 토글 상태

  useEffect(() => {
    // 예시로 사용자의 닉네임과 농작물 데이터를 가져오는 로직
    setNickName('사용자'); // 실제로는 API 등을 통해 데이터를 가져옴
    setMyCrops([{ id: 1, name: '작물1' }, { id: 2, name: '작물2' }]); // 실제로는 API 등을 통해 데이터를 가져옴
    setReceivedCrops([{ id: 3, name: '받은작물1' }, { id: 4, name: '받은작물2' }]); // 예시 데이터
  }, []);

  const handleToggle = (showMyCrops: boolean) => {
    setIsMyCrops(showMyCrops);
  };

  return (
    <div className={cx('cropStorageContainer')}>
      <h1>{nickName}님</h1>
      <h1>
        {isMyCrops
          ? `나의 작물 ${myCrops.length}개`
          : `물물교환 / 나눔 작물 ${receivedCrops.length}개`}
      </h1>
      <p>
        {isMyCrops
          ? '나의 농작물을 선택하면 해당 농작물의 농사일지를 볼 수 있어요'
          : <>물물교환 / 나눔 받은 작물이에요. <br />
           받은 농작물을 선택하면 해당 농작물의 농사일지를 볼 수 있어요</>}
      </p>
      <div className={cx('storageImage')}>
        <img src={Storage} alt="Storage" />
      </div>
      <div className={cx('toggleContainer')}>
        <button
          className={cx('toggleButton', { active: isMyCrops })}
          onClick={() => handleToggle(true)}
        >
          나의 농작물
        </button>
        <button
          className={cx('toggleButton', { active: !isMyCrops })}
          onClick={() => handleToggle(false)}
        >
          받은 농작물
        </button>
      </div>
      {isMyCrops ? (
        <div className={cx('myCropsContainer')}>
          <MyCrops crops={myCrops} />
        </div>
      ) : (
        <div className={cx('receivedCropsContainer')}>
          받은 농작물
        </div>
      )}
    </div>
  );
}

export default CropStoragePage;
