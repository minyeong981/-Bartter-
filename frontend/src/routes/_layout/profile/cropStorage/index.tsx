...
export const Route = createFileRoute('/_layout/profile/cropStorage/')({
  component: CropStoragePage,
});

function CropStoragePage() {
  const [nickName, setNickName] = useState(''); // 사용자 닉네임
  const [myCrops, setMyCrops] = useState<Crop[]>([]); // 나의 농작물 데이터
  const [receivedCrops, setReceivedCrops] = useState<Crop[]>([]); // 받은 농작물 데이터
  const [isMyCrops, setIsMyCrops] = useState(true); // 토글 상태

  useEffect(() => {
    // 예시로 사용자의 닉네임과 농작물 데이터를 가져오는 로직
    setNickName('사용자'); // 실제로는 API 등을 통해 데이터를 가져옴
    setMyCrops([{ id: 1, nickname: '작물1',date:'1',description:'',image:'',name:'' }, { id: 2, nickname: '작물2',date:'1',description:'',image:'',name:'' }]); // 실제로는 API 등을 통해 데이터를 가져옴
    setReceivedCrops([{ id: 3, nickname: '받은작물1',date:'1',description:'',image:'',name:'' }, { id: 4, nickname: '받은작물2',date:'1',description:'',image:'',name:'' }]); // 예시 데이터
  }, []);
