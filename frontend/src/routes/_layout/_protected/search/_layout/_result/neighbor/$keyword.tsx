...
export default function NeighborResultPage() {
  ...
  const { data, fetchNextPage } =  useSuspenseInfiniteQuery({
    ...
    queryFn: ({pageParam}) => barter.getUserListByKeyword(keyword, pageParam, PAGE_LIMIT),
    initialPageParam:0,
  ...
  return (
    ...
    <div className={cx('neighborList')} ref={rootElementRef}>
      {searchResultData.length ? searchResultData.map(((user, index) => <NeighborListCard key={index} {...user} />)) :
