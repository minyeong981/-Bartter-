...
export default function CommunityResultPage() {
  ...
  const { data, fetchNextPage } =  useSuspenseInfiniteQuery({
    ...
    queryFn: ({pageParam}) => barter.getCommunityPostListByKeyword(keyword, pageParam, PAGE_LIMIT),
    initialPageParam:0,
  ...
  return (
    <>
    <HeaderWithLabelAndBackButton label="동네 모임" style={{position: "static"}}/>
    <div className={cx('result-box')}>
      <span className={cx('result-text')}>{keyword}</span>
      검색 결과
    </div>
    <div className={cx('postList')} ref={rootElementRef}>
      {searchResultData.length ? searchResultData.map(((trade, index) => <PostCard key={index} {...trade} />)) :
